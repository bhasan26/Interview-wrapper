...
const askClaudeButton = document.querySelector('#ask-claude');
...
function showCaptureIndicator() {
  window.clearTimeout(captureIndicatorTimeout);
  captureIndicator.classList.add('is-visible');
  captureIndicatorTimeout = window.setTimeout(() => {
    captureIndicator.classList.remove('is-visible');
  }, 1000);
}
...
function renderCodeBlock(code, language) {
  ...
  return `<div class="code-block"><button class="copy-code" type="button">Copy code</button><pre><code${languageClass}>${escapeHtml(code)}</code></pre></div>`;
}

function enhanceCodeBlocks() {
  for (const block of answerContent.querySelectorAll('pre code')) {
    if (window.hljs) {
      window.hljs.highlightElement(block);
    }
  }

  for (const button of answerContent.querySelectorAll('.copy-code')) {
...
askClaudeButton.addEventListener('click', () => window.interviewOverlay.askClaude(questionText.value));
...
window.interviewOverlay.onScreenshotCaptured(showCaptureIndicator);
